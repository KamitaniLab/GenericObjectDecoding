function [Model, Info] = linear_sparse_space_vec_sw(X,Y,Model,parm)
...
fprintf('Linear sparse space (vector output & variable trial) start\n')
...
%	    A  = ((WW./SX) + 2*Ta0*a0)./(G_A*N*D + 2*Ta0);
    ...
    if mod(k, Nskip)==0
        ...
        fprintf('Iter=%4d, M =%4d, err=%g, F=%g, H=%g, SY=%g, Wmin=%g\n',...
        		        ...
        		        sum(SY), min(sum(WW,2))/max(sum(WW,2)));
    end
